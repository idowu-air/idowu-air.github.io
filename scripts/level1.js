/*Script for level 1: 90s cartoons
...
function resetCards() {
  ...
  for (let i = 0; i < cards.length; i++) {
    $(".game-container-level1").append($(cards[i].outerHTML));
  }
}
...
const gameState = {
  ...
  resetFlippedCards: function() {
    ...
    // Necessary to allow users who click too fast to still see the second card
    sleep(200);
  },
  disableCorrectCards: function() {
    // Disable click functionality from already matched card pairs
...
$(document).ready(function() {
  ...
  $(".card").click(function () {
    ...
    $(this).on("flip:done", function () {
      // We only compare cards if we have a pair (2), else this block does not run
      if (gameState.currentFlippedCards.length == 2) {
        if (gameState.checkFlippedCards() == true) {
          gameState.correctMatches += 1;
          gameState.disableCorrectCards();
          $("#correct-pairs").text(`${gameState.correctMatches}/4`);
